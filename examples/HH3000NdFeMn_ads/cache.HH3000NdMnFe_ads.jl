...
mutable struct Reactran{T,chunk_size}
    Mn::PreallocationTools.DiffCache{
        Array{T,1},
        Array{ForwardDiff.Dual{nothing,T,chunk_size},1},
    }
    Mn_tran::PreallocationTools.DiffCache{
        Array{T,1},
        Array{ForwardDiff.Dual{nothing,T,chunk_size},1},
    }
    Mn_ads_Mn::PreallocationTools.DiffCache{
        Array{T,1},
        Array{ForwardDiff.Dual{nothing,T,chunk_size},1},
    }
    Mn_ads_Mn_tran::PreallocationTools.DiffCache{
        Array{T,1},
        Array{ForwardDiff.Dual{nothing,T,chunk_size},1},
    }
    Mn_ads_Fe::PreallocationTools.DiffCache{
        Array{T,1},
        Array{ForwardDiff.Dual{nothing,T,chunk_size},1},
    }
    Mn_ads_Fe_tran::PreallocationTools.DiffCache{
        Array{T,1},
        Array{ForwardDiff.Dual{nothing,T,chunk_size},1},
    }
    ...
    Fe_tran::PreallocationTools.DiffCache{
        Array{T,1},
        Array{ForwardDiff.Dual{nothing,T,chunk_size},1},
    }
    Fe_ads_Mn::PreallocationTools.DiffCache{
        Array{T,1},
        Array{ForwardDiff.Dual{nothing,T,chunk_size},1},
    }
    Fe_ads_Mn_tran::PreallocationTools.DiffCache{
        Array{T,1},
        Array{ForwardDiff.Dual{nothing,T,chunk_size},1},
    }
    Fe_ads_Fe::PreallocationTools.DiffCache{
        Array{T,1},
        Array{ForwardDiff.Dual{nothing,T,chunk_size},1},
    }
    Fe_ads_Fe_tran::PreallocationTools.DiffCache{
        Array{T,1},
        Array{ForwardDiff.Dual{nothing,T,chunk_size},1},
    }
    ...
    Mn_free::PreallocationTools.DiffCache{
        Array{T,1},
        Array{ForwardDiff.Dual{nothing,T,chunk_size},1},
    }
    ...
    RMnRe_Ndnr::PreallocationTools.DiffCache{
        Array{T,1},
        Array{ForwardDiff.Dual{nothing,T,chunk_size},1},
    }
    RMnRe_Ndr::PreallocationTools.DiffCache{
        Array{T,1},
        Array{ForwardDiff.Dual{nothing,T,chunk_size},1},
    }
    RMnOx_Ndnr::PreallocationTools.DiffCache{
        Array{T,1},
        Array{ForwardDiff.Dual{nothing,T,chunk_size},1},
    }
    RMnOx_Ndr::PreallocationTools.DiffCache{
        Array{T,1},
        Array{ForwardDiff.Dual{nothing,T,chunk_size},1},
    }
    RFeRe_Ndnr::PreallocationTools.DiffCache{
        Array{T,1},
        Array{ForwardDiff.Dual{nothing,T,chunk_size},1},
    }
    RFeRe_Ndr::PreallocationTools.DiffCache{
        Array{T,1},
        Array{ForwardDiff.Dual{nothing,T,chunk_size},1},
    }
    RFeOx_Ndnr::PreallocationTools.DiffCache{
        Array{T,1},
        Array{ForwardDiff.Dual{nothing,T,chunk_size},1},
    }
    RFeOx_Ndr::PreallocationTools.DiffCache{
        Array{T,1},
        Array{ForwardDiff.Dual{nothing,T,chunk_size},1},
    }
    ...
    S_TMn::PreallocationTools.DiffCache{
        Array{T,1},
        Array{ForwardDiff.Dual{nothing,T,chunk_size},1},
    }
    ...
    S_TFe::PreallocationTools.DiffCache{
        Array{T,1},
        Array{ForwardDiff.Dual{nothing,T,chunk_size},1},
    }
    ...
    S_SurfMn_Ndnr::PreallocationTools.DiffCache{
        Array{T,1},
        Array{ForwardDiff.Dual{nothing,T,chunk_size},1},
    }
    S_SurfMn_Ndr::PreallocationTools.DiffCache{
        Array{T,1},
        Array{ForwardDiff.Dual{nothing,T,chunk_size},1},
    }
    S_SurfFe_Ndnr::PreallocationTools.DiffCache{
        Array{T,1},
        Array{ForwardDiff.Dual{nothing,T,chunk_size},1},
    }
    S_SurfFe_Ndr::PreallocationTools.DiffCache{
        Array{T,1},
        Array{ForwardDiff.Dual{nothing,T,chunk_size},1},
    }
...
function init(
    ...
    Mn = PreallocationTools.dualcache(zeros(T, Ngrid), Val{chunk_size})
    Mn_tran = PreallocationTools.dualcache(zeros(T, Ngrid), Val{chunk_size})
    Mn_ads_Mn = PreallocationTools.dualcache(zeros(T, Ngrid), Val{chunk_size})
    Mn_ads_Mn_tran =
        PreallocationTools.dualcache(zeros(T, Ngrid), Val{chunk_size})
    Mn_ads_Fe = PreallocationTools.dualcache(zeros(T, Ngrid), Val{chunk_size})
    Mn_ads_Fe_tran =
        PreallocationTools.dualcache(zeros(T, Ngrid), Val{chunk_size})
    Fe = PreallocationTools.dualcache(zeros(T, Ngrid), Val{chunk_size})
    Fe_tran = PreallocationTools.dualcache(zeros(T, Ngrid), Val{chunk_size})
    Fe_ads_Mn = PreallocationTools.dualcache(zeros(T, Ngrid), Val{chunk_size})
    Fe_ads_Mn_tran =
        PreallocationTools.dualcache(zeros(T, Ngrid), Val{chunk_size})
    Fe_ads_Fe = PreallocationTools.dualcache(zeros(T, Ngrid), Val{chunk_size})
    Fe_ads_Fe_tran =
        PreallocationTools.dualcache(zeros(T, Ngrid), Val{chunk_size})
    ...
    Mn_free = PreallocationTools.dualcache(zeros(T, Ngrid), Val{chunk_size})
    Omega_RFeS_dis =
        PreallocationTools.dualcache(zeros(T, Ngrid), Val{chunk_size})
    Omega_RFeS_pre =
        PreallocationTools.dualcache(zeros(T, Ngrid), Val{chunk_size})
    ...
    RMnRe_Ndnr = PreallocationTools.dualcache(zeros(T, Ngrid), Val{chunk_size})
    RMnRe_Ndr = PreallocationTools.dualcache(zeros(T, Ngrid), Val{chunk_size})
    RMnOx_Ndnr = PreallocationTools.dualcache(zeros(T, Ngrid), Val{chunk_size})
    RMnOx_Ndr = PreallocationTools.dualcache(zeros(T, Ngrid), Val{chunk_size})
    RFeRe_Ndnr = PreallocationTools.dualcache(zeros(T, Ngrid), Val{chunk_size})
    RFeRe_Ndr = PreallocationTools.dualcache(zeros(T, Ngrid), Val{chunk_size})
    RFeOx_Ndnr = PreallocationTools.dualcache(zeros(T, Ngrid), Val{chunk_size})
    RFeOx_Ndr = PreallocationTools.dualcache(zeros(T, Ngrid), Val{chunk_size})
    ...
    S_TMn = PreallocationTools.dualcache(zeros(T, Ngrid), Val{chunk_size})
    S_FeOOH = PreallocationTools.dualcache(zeros(T, Ngrid), Val{chunk_size})
    S_TFe = PreallocationTools.dualcache(zeros(T, Ngrid), Val{chunk_size})
