using Plots.PlotMeasures,Printf,DataStructures

include("helpers.jl")
include("pH_helpers.jl")


function generate_output_old(site::Vector{String},ylim=L,saveplt=false)
    ...
    for i in eachrow(plotting)
        if i.name != "TA"
            ...
            i.name : i.expression)" ) # if expression given
    ...
    profile_raw = OrderedDict(profile_name .=> [Base.invokelatest.(f,input_profile...) for f in profile_function])
    ...
    output = OrderedDict(plotting.name .=> [(profile=profile_raw[i.name].*eval(Meta.parse(i.conversion_profile)),unit_profile=i.unit_profile) for i in eachrow(plotting)])
...
function calc_flux_top(φ, D, u, x, C, BC)
    ...
    c0+= (-x1^3*x2^2*x3+x1^2*x2^3*x3+x1^3*x2*x3^2-x1*x2^3*x3^2-x1^2*x2*x3^3+x1*x2^2*x3^3)*γ⁰
    c0/= (x1-x2)*(x2-x3)*(-x1+x3)*(x1*x2*x3*α⁰-x1*x2*β⁰-x1*x3*β⁰-x2*x3*β⁰)

    dCdz0 = c3*(x1^3*x2^2*α⁰-x1^2*x2^3*α⁰)
    dCdz0+= c2*(-x1^3*x3^2*α⁰+x1^2*x3^3*α⁰)
    dCdz0+= c1*(x2^3*x3^2*α⁰-x2^2*x3^3*α⁰)
    dCdz0+= (-x1^3*x2^2+x1^2*x2^3+x1^3*x3^2-x2^3*x3^2-x1^2*x3^3+x2^2*x3^3)*γ⁰
    dCdz0/= (x1-x2)*(x2-x3)*(x1-x3)*(x1*x2*x3*α⁰-x1*x2*β⁰-x1*x3*β⁰-x2*x3*β⁰)
